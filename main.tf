...
resource "aws_s3_bucket" "terraform_state" {
  bucket = "terraform-up-and-running-chokk"         // name of the bucket name that is globally unique

  #to prevent accidental deletion of this S3 bucket LIFECYCLE
  lifecycle{
      prevent_destroy = true
  }

  #enable versioning so the full version hist of our statefiles can be viewed
  versioning {
      enabled = true
  }
...
#Creating the Dynamodb Table for locking with Terraform execution
